...
class C_app_job extends CI_Controller {
	protected $_ci;

	public function __construct(){
		parent::__construct();
		$this->_ci = &get_instance();
		$this->load->model('App_job');
	}
	...
	public function get($criteria = null){
		$this->App_job->set_database($this->load->database('default',TRUE));

		$App_job = $this->App_job->get("*", $this->input->get('params'),$this->input->get('limit'),$this->input->get('start'));
		echo json_encode(
			array(
				'results' 	=> $App_job->result(),
				'total' 	=> $App_job->num_rows(),
				'status'	=> 200,
			)
		);
	}
	...
	public function save(){
		...
		$this->App_job->set_database($this->load->database('default',TRUE));
		$parameter['role_id'] = $this->App_job->get(" max(role_id) as role_id");
		if ($parameter['role_id']->num_rows() > 0) {
			$parameter['role_id'] = $parameter['role_id']->row()->role_id + 1;
		}

		$result = $this->App_job->create($parameter);
	...
	public function update(){
		...
		$this->App_job->set_database($this->load->database('default',TRUE));
		$result = $this->App_job->update($criteria, $parameter);
		if ($result > 0 || $result === true) {
			$response['message'] 	= "Berhasil disimpan";
			$response['status'] 	= 200;
		}else{
	...
	public function delete(){
		...
		if (count(json_decode($parameter['id'])) > 0) {
			foreach (json_decode($parameter['id']) as $key => $value) {
				...
				$response['status'] 	= $this->App_job->delete($criteria);

...
class C_app_tenant extends CI_Controller {
	protected $_ci;

	public function __construct(){
		parent::__construct();
		$this->_ci = &get_instance();
		$this->load->model('App_tenant');
	}
	...
	public function get($criteria = null){
		$this->App_tenant->set_database($this->load->database('default',TRUE));

		$App_tenant = $this->App_tenant->get("*", $this->input->get('params'),$this->input->get('limit'),$this->input->get('start'));
		echo json_encode(
			array(
				'results' 	=> $App_tenant->result(),
				'total' 	=> $App_tenant->num_rows(),
				'status'	=> 200,
			)
		);
	}
	...
	public function save(){
		...
		$this->App_tenant->set_database($this->load->database('default',TRUE));
		$parameter['role_id'] = $this->App_tenant->get(" max(role_id) as role_id");
		if ($parameter['role_id']->num_rows() > 0) {
			$parameter['role_id'] = $parameter['role_id']->row()->role_id + 1;
		}

		$result = $this->App_tenant->create($parameter);
	...
	public function update(){
		...
		$this->App_tenant->set_database($this->load->database('default',TRUE));
		$result = $this->App_tenant->update($criteria, $parameter);
		if ($result > 0 || $result === true) {
			$response['message'] 	= "Berhasil disimpan";
			$response['status'] 	= 200;
		}else{
	...
	public function delete(){
		...
		if (count(json_decode($parameter['id'])) > 0) {
			foreach (json_decode($parameter['id']) as $key => $value) {
				...
				$response['status'] 	= $this->App_tenant->delete($criteria);

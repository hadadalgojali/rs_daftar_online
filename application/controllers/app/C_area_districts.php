...
class C_Area_districts extends CI_Controller {
	protected $_ci;

	public function __construct(){
		parent::__construct();
		$this->_ci = &get_instance();
		$this->load->model('Area_districts');
	}
	...
	public function get($criteria = null){
		$this->Area_districts->set_database($this->load->database('default',TRUE));

		$result = $this->Area_districts->get("*", $this->input->get('params'), $this->input->get('limit'), $this->input->get('start'));
		echo json_encode(
			array(
				'results' => $result->result(),
				'total' 	=> $this->Area_districts->get(" COALESCE(count(*),0) as count ")->row()->count,
				'status'	=> 200,
			)
		);
	}
	...
	public function save(){
		...
		$this->Area_districts->set_database($this->load->database('default',TRUE));
		$parameter['country_id'] = $this->Area_districts->get(" max(country_id) as id");
		if ($parameter['country_id']->num_rows() > 0) {
			$parameter['country_id'] = $parameter['country_id']->row()->id + 1;
		}

		$result = $this->Area_districts->create($parameter);
	...
	public function update(){
		...
		$this->Area_districts->set_database($this->load->database('default',TRUE));
		$result = $this->Area_districts->update($criteria, $parameter);
		if ($result['result'] > 0 || $result['result'] === true) {
			$response['status'] 	= 200;
	...
	public function delete(){
		...
		if (count(json_decode($parameter['id'])) > 0) {
			foreach (json_decode($parameter['id']) as $key => $value) {
				...
				$result 	= $this->Area_districts->delete($criteria);

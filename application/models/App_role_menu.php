...
class App_role_menu extends CI_Model {
	...
	public function get_with_menu($select = "*", $criteria = null){
		$this->database->select($select);
		$this->database->from($this->table);
		if ($criteria !== null) {
			$this->database->where($criteria);
		}
		$this->database->join("app_menu", " app_menu.menu_id = ".$this->table.".menu_id ", "INNER");
		$this->database->order_by("app_menu.menu_name", "ASC");
		return $this->database->get();
	}
